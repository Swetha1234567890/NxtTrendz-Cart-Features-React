...
class App extends Component {
  ...
  incrementCartItemQuantity = id => {
    this.setState(prevState => ({
      cartList: prevState.cartList.map(each => {
        if (each.id === id) {
          const updatedIncrementQuantity = each.quantity + 1
          return {...each, quantity: updatedIncrementQuantity}
        }
        return each
      }),
    }))
  ...
  decrementCartItemQuantity = id => {
    ...
    if (productObject.quantity > 1) {
      this.setState(prevState => ({
        cartList: prevState.cartList.map(each => {
          if (each.id === id) {
            const updatedDecrementQuantity = each.quantity - 1
            return {...each, quantity: updatedDecrementQuantity}
          }
          return each
        }),
      }))
